...
int add_memory_for_matrices(double ***matrix_a, double ***matrix_b,
                            double ***matrix_result, size_t n_a, size_t m_a,
                            size_t n_b, size_t m_b) {
...
void write_result_to_file(const string &filename, size_t size,
...
int time_tests(size_t cnt_times, size_t measurements, bool verbose) {
  ...
  unsigned long long start, end, total_simple, total_vinograd,
      total_vinograd_update;
  const string out_file = "results.txt";

  remove(out_file.c_str());
  if (!verbose) {
    cout << left << setw(12) << "Matrix Size" << "|" << setw(15)
         << "Simple (cycles)" << "|" << setw(17) << "Vinograd (cycles)" << "|"
         << setw(22) << "Vinograd update (cycles)" << endl;

    cout << string(12, '-') << "+" << string(15, '-') << "+" << string(17, '-')
         << "+" << string(22, '-') << endl;
  }

  for (size_t i = 1; i < cnt_times; i++) {
    rc = add_memory_for_matrices(&matrix_a, &matrix_b, &matrix_result, i, i, i,
                                 i);
    if (rc != OK) {
      if (!verbose)
        cout << "Error: Failed to allocate memory for " << i << "x" << i
             << " matrices" << endl;
      return rc;
    }
    ...
    if (!data) {
      ...
      if (!verbose)
        cout << "Error: Failed to allocate data array for size " << i << "x"
             << i << endl;
      return ERROR_ADD_MEM;
    }
    ...
    if (!verbose) {
      cout << left << setw(12) << (to_string(i) + "x" + to_string(i)) << "|"
           << setw(15) << avg_simple << "|" << setw(17) << avg_vinograd << "|"
