...
\renewcommand{\labelenumii}{\arabic{enumi}.\arabic{enumii}\text{)}}
...
\captionsetup[table]{labelsep=dash,justification=raggedleft,singlelinecheck=off}
...
\lstset{ %
	language=caml,                 % выбор языка для подсветки				(caml - C, C++ -- C++, [x86masm]Assembler, Python, SQL)
	basicstyle=\small\ttfamily, % размер и начертание шрифта для подсветки кода
	numbers=left,               % где поставить нумерацию строк (слева\справа)             (left none)
	numberstyle=\tiny,           % размер шрифта для номеров строк                         (\tiny либо убрать)
...
%	numbersep=5pt,                % как далеко отстоят номера строк от подсвечиваемого кода
	...
	tabsize=2,                 % размер табуляции по умолчанию равен 2 пробелам			(а не 4?)
	captionpos=t,              % позиция заголовка вверху [t] или внизу [b]             (b)
	breaklines=true,           % автоматически переносить строки (да\нет)
	breakatwhitespace=false, % переносить строки только если есть пробел 				(true)
